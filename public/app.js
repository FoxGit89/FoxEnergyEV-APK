...
// ============================================================
// SESSIONE SICURA — durata in secondi
// ============================================================
...
const secureSession = {
  ...
  _boundVisibility: null, _boundUnload: null,
  ...
  startSession(slotLabels) {
    ...
    localStorage.setItem(this.STORAGE_KEY, JSON.stringify({
      startedAt: this._startedAt, slotLabels, userId: app.user.telegramId
    }));
  ...
  _renderSessionScreen() {
    ...
    if (s) s.textContent = this._slotLabels.length===1
      ? `Slot caricato: ${this._slotLabels[0]}`
      : `Slot caricati: ${this._slotLabels.join(', ')}`;
    const m = document.getElementById('session-status-msg');
    if (m) m.textContent = 'Avvicinati alla colonnina e usa il Chameleon Ultra. Gli slot verranno cancellati automaticamente alla scadenza.';
    const b = document.getElementById('session-warning-bar'); if (b) b.classList.add('hidden');
    const btn = document.getElementById('session-cancel-btn'); if (btn) btn.disabled = false;
  ...
  _startTimer() {
    ...
    this._timer = setInterval(() => {
      ...
      if (remaining <= 0) { clearInterval(this._timer); this._timer=null; this.cancelSession('auto'); }
    }, 500);
  },
  ...
  _updateCountdown(remaining) {
    ...
    numEl.className = `session-countdown-num${urgent?' urgent':''}`;
    if (ring) { ring.style.strokeDashoffset = 326.73*(1-remaining/SESSION_TIMEOUT_SEC); ring.className=`session-ring-fill${urgent?' urgent':''}`; }
    if (bar && secs) {
      if (urgent && remaining>0) { bar.classList.remove('hidden'); secs.textContent=remaining; }
  ...
  async cancelSession(reason) {
    ...
    const btn = document.getElementById('session-cancel-btn'); if (btn) btn.disabled=true;
    ...
    this._updateWipeUI('🗑️', 'CANCELLAZIONE IN CORSO', `${reason==='auto'?'Timer scaduto':'Cancellazione manuale'}...`);
    try {
  ...
  checkPendingSession() {
    const raw = localStorage.getItem(this.STORAGE_KEY); if (!raw) return false;
    try {
      const s = JSON.parse(raw);
      const elapsed = Math.floor((Date.now()-(s.startedAt||0))/1000);
      if (s.userId !== app.user.telegramId) { localStorage.removeItem(this.STORAGE_KEY); return false; }
      if (elapsed < SESSION_TIMEOUT_SEC+30) { this._slotLabels=s.slotLabels||[]; this._startedAt=s.startedAt; return true; }
  ...
  resumePendingSession() {
    ...
    if (remaining<=0) this.cancelSession('auto');
      ...
      this._updateCountdown(remaining); this._startTimer();
      document.addEventListener('visibilitychange', this._boundVisibility=this._onVisibilityChange.bind(this));
      window.addEventListener('beforeunload', this._boundUnload=this._onBeforeUnload.bind(this));
    }
  },
...
const app = {
  ...
  async apiCall(params) {
    ...
    try {
      ...
      try { return JSON.parse(text); }
        ...
        const fb=text.indexOf('{'), fb2=text.indexOf('['); let start=-1;
        if(fb!==-1&&fb2!==-1) start=Math.min(fb,fb2); else if(fb!==-1) start=fb; else if(fb2!==-1) start=fb2;
        if(start!==-1){const isArr=text[start]==='[',end=isArr?text.lastIndexOf(']'):text.lastIndexOf('}');if(end>start) return JSON.parse(text.substring(start,end+1));}
  ...
  async loadDashboard() {
    ...
    try {
      ...
      this.renderDashboard(); this.renderSlots();
  ...
  renderNotifications() {
    ...
    const ico={warning:'⚠️',info:'ℹ️',success:'✅',danger:'❌'}, src={admin:'Fox Energy',system:'Sistema'};
  ...
  async showHistory() {
    ...
    } catch(e) { list.innerHTML=`<div class="notif-empty"><div style="font-size:48px;margin-bottom:16px">❌</div><div style="color:#FF5252;font-size:15px">Errore</div></div>`; }
  },
...
// ============================================================
// BLE ENGINE — IDENTICO ALL'ORIGINALE CHE FUNZIONAVA
// Non modificare: usa window.ChameleonUltraJS esattamente
// come nella versione originale funzionante.
// ============================================================
window.bleEngine = {
  ultra: null,
  ...
  async startSync(mapping, telegramId) {
    ...
    try {
      ...
      this.updateUI(5, "🧹 Pulizia di tutti gli 8 slot...", 'working');
      for (let i=0; i<8; i++) {
      ...
      for (let idx=0; idx<total; idx++) {
        ...
        // Rilevamento automatico tipo tessera (1K/4K/Ultralight/NTAG)
        ...
        this.updateUI(base+(80/total)*0.25, `[${idx+1}/${total}] ⚙️ Config slot ${slotIdx+1}...`, 'working');
      ...
      // NON disconnettere — sessione sicura mantiene BLE aperto
  ...
  // wipeAllSlots: riusa this.ultra se ancora connesso, altrimenti
  // crea nuova istanza con lo stesso pattern originale
  async wipeAllSlots(updateCallback) {
    const { TagType, FreqType } = window.ChameleonUltraJS;
    updateCallback('🗑️', 'CANCELLAZIONE IN CORSO', 'Verifica connessione...');
