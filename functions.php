...
function getUserMonthlyStats($telegram_id) {
    ...
    try {
        ...
        $s->execute([$telegram_id, $month]); $r = $s->fetch(PDO::FETCH_ASSOC);
        if ($r && $r['total']) $stats['recharged_eur'] = number_format((float)$r['total'], 2, '.', '');
    } catch(Exception $e) { $errori[] = 'Wallet: '.$e->getMessage(); }
    try {
        ...
        $s->execute([$telegram_id, $month]); $r = $s->fetch(PDO::FETCH_ASSOC);
        if ($r) { if ($r['tot_kwh']) $stats['energy_kwh'] = number_format((float)$r['tot_kwh'],2,'.','' ); if ($r['tot_eur']) $stats['spent_eur'] = number_format((float)$r['tot_eur'],2,'.','' ); }
    } catch(Exception $e) { $errori[] = 'Transactions: '.$e->getMessage(); }
    if ($errori) $stats['debug_msg'] = implode(' | ', $errori);
    return $stats;
}
?>