...
function getParam(string $key, string $default=''): string {
    static $body=null;
    if ($body===null) { $r=file_get_contents('php://input'); $body=($r&&$r!=='')?json_decode($r,true)??[]:[];}
...
if ($action==='login') {
    ...
    $s=db()->prepare("SELECT telegram_id,first_name FROM users WHERE (username=? OR telegram_id=?) AND card_number=?"); $s->execute([$input,$input,$card]); $u=$s->fetch(PDO::FETCH_ASSOC);
    if ($u) {
...
if (!$user_id) { echo json_encode(['error'=>'User ID mancante.']); exit; }
...
try {
    ...
    if ($action==='get_dashboard') {
        ...
        } catch(\Throwable $e){ echo json_encode(['error'=>'CRASH: '.$e->getMessage()]); }
        exit;
    }

    // ── SLOTS (fix: parametri bindati senza db()->quote) ──
    if ($action==='get_slots') {
        $u_tg=(string)$user['telegram_id']; $u_name=(string)($user['username']??'');
        $u_score=(int)($user['trust_score']??100); $u_prem=(int)($user['is_premium']??0);
    ...
    if ($action==='get_json_content') {
        $fid=$_GET['file_id']??''; if(!$fid){echo json_encode(['error'=>'File ID mancante.']);exit;}
    ...
    if ($action==='get_history') {
        try {
            $limit=min((int)($_GET['limit']??10),50); $sessions=[]; $transactions=[];
    ...
    if ($action==='start_session') {
        ...
        try { db()->prepare("INSERT INTO active_sessions(user_id,slots,status)VALUES(?,?,'active')")->execute([$user_id,$_GET['slots']??'']); echo json_encode(['success'=>true]); } catch(Exception $e){ echo json_encode(['success'=>true]); }
        exit;
    }
